		...
		<div class="row layout-top-spacing" id="cancel-row">
			<div id="ftFormArray" class="col-lg-12 layout-spacing">
				...
				<div class="statbox widget box box-shadow">
							...
							<table class="table table-bordered mb-4 table-hover">
								<tbody>
									<tr>
										...
										<td width="2%">:</td>
										<td>{{$record_data->post_title}}</td>
									</tr>
									<tr>
										<td>Details</td>
										<td>:</td>
										<td>{{$record_data->post_details}}</td>
									</tr>
									<tr>
										<td>Meeting url</td>
										<td>:</td>
										<td>{{$record_data->meeting_url}}</td>
									</tr>
									<tr>
										<td>Subject</td>
										<td>:</td>
									...
									<tr>
										<td>Status</td>
										<td>:</td>
										<td>@if($record_data->post_status==106)Closed @elseif($record_data->post_status==107)Solved @else Open @endif</td>
									</tr>
									<tr>
										<td>Admin Status</td>
										<td>:</td>
					...
					<hr>
					@if(count($record_data->post_images) > 0)
						<div class="widget-header">                                
							<div class="row">
								<div class="col-xl-12 col-md-12 col-sm-12 col-12">
									<h4>Post Images & Docs</h4> 
								</div>
							</div>
						</div>
						<div class="widget-content widget-content-area custom-autocomplete h-100">
							<div class="row">
								@foreach($record_data->post_images as $file_key => $file)
									@php
										$src_file_name = $file->post_pic;
										$supported_image = array('gif', 'jpg', 'jpeg', 'png', 'webp');
										$ext = strtolower(pathinfo($src_file_name, PATHINFO_EXTENSION));
										if(in_array($ext, $supported_image)){
											$file_icon = asset('assets/img/image.png');
										}else{
											if($ext=='pdf'){
												$file_icon = asset('assets/img/pdf.png');
											}elseif($ext=='doc' || $ext=='docx'){
												$file_icon = asset('assets/img/word.png');
											}elseif($ext=='xls' || $ext=='xlsx' || $ext=='csv'){
												$file_icon = asset('assets/img/excel.png');
											}
										}
									@endphp
									<div class="form-group col-2 custom-file-container">
										<a href="{{asset('storage/post_images/'.$src_file_name)}}" target="_blank">
											<img src="{{$file_icon}}" alt="file"> <!-- {!!$file_icon!!} -->
										</a>
									</div>
								@endforeach
							</div>
